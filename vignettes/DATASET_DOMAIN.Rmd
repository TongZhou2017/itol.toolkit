...
vignette: >
  %\VignetteIndexEntry{DATASET_DOMAIN}
...
template_start_group <- tab_tmp_01_long %>%
                        ...
                        spread(key=variable,value=sublen)
...
template_start_group[template_start_group == 0] <- NA # 既然要做加法，为什么还搞NA？
template_end_group <- template_start_group + start_group$start #第一列的问题
...
template_end_group_long <- template_end_group_long %>%
                           na.omit()
template_end_group_long$length <- sum(start_group$Freq)
template_end_group_long <- template_end_group_long[,c(2,5,4,3,1)] #排序等下看看
